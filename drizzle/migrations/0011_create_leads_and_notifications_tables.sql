...
-- Seed demo notifications only if the referenced profile exists
INSERT INTO notifications (user_id, type, title, message, link_tab, created_at)
SELECT * FROM (VALUES
('0fe28a04-eb90-42d4-ab64-99e3b051e3a1'::uuid, 'nouvelle_demande', 'Nouvelle demande urgente', 'Isabelle Roux a envoyé une demande urgente pour sa mère (retour hôpital demain).', 'leads', now() - interval '30 minutes'),
('0fe28a04-eb90-42d4-ab64-99e3b051e3a1'::uuid, 'parcours', 'Blocage sur le parcours Dupont', 'Le matelas anti-escarres n''est pas disponible. Étape 7 bloquée.', 'pathway', now() - interval '1 day'),
('0fe28a04-eb90-42d4-ab64-99e3b051e3a1'::uuid, 'transmission', 'Transmission urgente', 'Dr. Lambert a mis à jour l''ordonnance de Mme Dupont (anticoagulant + antalgiques).', 'pathway', now() - interval '2 hours')
) AS v(user_id, type, title, message, link_tab, created_at)
WHERE EXISTS (SELECT 1 FROM profiles WHERE id = '0fe28a04-eb90-42d4-ab64-99e3b051e3a1');
